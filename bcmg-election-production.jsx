...
const DEFAULT_CANDIDATE = {
  id: 1, name: "ADV. JAYANT JAIBHAVE", ballot_no: "61",
...
function localSearch(q, type) {
  ...
  return DEMO_VOTERS.filter(v => {
    ...
    if (type === "mobile") return (v.mobile || "").includes(q.trim());
    ...
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        {candidate.photo_url ? (
          ...
          <div style={{ width: 46, height: 46, borderRadius: "50%", background: C.goldDim, display: "flex", alignItems: "center", justifyContent: "center", border: `2px solid ${C.gold}`, fontSize: 18, fontWeight: 800, color: C.gold, fontFamily: font }}>
...
const VoterModal = ({ voter, candidate, onClose, onLogEvent }) => {
  ...
  const sendToVoter = () => {
    ...
    onLogEvent?.("whatsapp_send", { voterId: voter.id, voterName: voter.name, voterEnrolment: voter.enrolment, boothName: voter.booth_name });
  };

  const shareSlip = () => {
    window.open(`https://wa.me/?text=${encodeURIComponent(genMsg())}`, '_blank');
    onLogEvent?.("whatsapp_share", { voterId: voter.id, voterName: voter.name });
  };
    ...
    <Modal onClose={onClose}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 9 }}>
          ...
          <Btn variant="greenOutline" onClick={shareSlip} style={{ padding: "13px 20px" }}>
            <Icon.WhatsApp /> Share via WhatsApp
          </Btn>
          <Btn variant="default" onClick={() => { onLogEvent?.("download_slip", { voterId: voter.id }); }} style={{ padding: "13px 20px" }}>
        ...
        <div style={{ marginTop: 16, paddingTop: 14, borderTop: `1px solid ${C.border}` }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            {[
              ...
              { icon: <Icon.Phone />, l: "Mobile", v: voter.mobile ? `+91 ${voter.mobile}` : "Not available" },
            ...
            {recentEvents.map((e, i) => {
              ...
              return (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 0", borderBottom: i < recentEvents.length - 1 ? `1px solid ${C.border}` : "none" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <span style={{ fontSize: 12, fontWeight: 600, color: C.text }}>{typeLabels[e.type] || e.type}</span>
                    {e.name && <span style={{ fontSize: 12, color: C.textDim }}> — {e.name}</span>}
                    {e.query && <span style={{ fontSize: 12, color: C.textDim }}> — "{e.query}"</span>}
                  </div>
...
export default function BCMGElectionPortal() {
  ...
  const logEvent = useCallback((type, details = {}) => {
    ...
    // In production: import { logEvent } from './lib/api'; logEvent(type, details);
  }, []);
  ...
  const doSearch = useCallback(() => {
    ...
    // const { data } = await searchVoters(query, searchType);
    // setResults(data);

    setTimeout(() => {
      const found = localSearch(query, searchType);
      setResults(found);
      setHasSearched(true);
      setLoading(false);
      logEvent("search", { searchQuery: query, searchType, resultsCount: found.length });
    }, 200);
  ...
  const searchTypes = [
    ...
    { key: "sr", label: "Sr. No.", icon: <Icon.Hash /> },
    { key: "mobile", label: "Mobile", icon: <Icon.Phone /> },
  ];
  ...
  if (page === "admin") {
    return (
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: fontSans, color: C.text }}>
        <TopBar onSettings={() => setShowSettings(true)} onAdmin={() => setPage("home")} isAdmin />
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, fontFamily: fontSans, color: C.text }}>
      ...
      <TopBar onSettings={() => setShowSettings(true)} onAdmin={() => setPage("admin")} />
...
//  TOP BAR
// ═══════════════════════════════════════════
function TopBar({ onSettings, onAdmin, isAdmin = false }) {
  return (
    <div style={{
